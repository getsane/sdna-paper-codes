...
def simpath(G, tol, l):
    ...
    Input: G(V,E) Networkx Directed Graph Object
    ...
    C = nx.min_weighted_vertex_cover(G)  # list of vertices
    ...
    for c in C:                              # line 2
        ...
        VminU = [v for v in V if v not in U] # V-v where 'v' belongs to U
        
        U_sg = G.subgraph(VminU)             # creating a subgraph of nodes V-u
        
        spd_u, spdW_v = simpath_spread(c, tol, U=U_sg)   # spread(u) on Graph of nodes: V-v = V
                                                         # will give both spread in G as well
                                                         # as G-S+u simultaneously 
        tup_c[c] = spdW_v                   # Adding to dictionary for use                                    
    ...
    while len(S) < k:                       # line 10
        ...
        u_flag += 1   # current iteration

        for ul in lu:
            ul_x = [v for v in V if v != ul]        # V - x; want to keep V intact.

            spd,spdV_xS = simpath_spread(S, tol, U = G.subgraph(ul_x))      
        
        for x,y in lu:
            if y == u_flag:                 # checking if x part of the previous iteration
                S = S + [x]                 
                spd = spd[x]
